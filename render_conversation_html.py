...
.questionnaire-list .question-text {{
  font-weight: 600;
  margin-bottom: 6px;
  color: var(--assistant);
}}
...
def render_questionnaire(questionnaire: Optional[Dict[str, Any]]) -> str:
    if not questionnaire:
        return ""
    questions = questionnaire.get("questions") or DEFAULT_QUESTION_LIST
    answers = questionnaire.get("answers") or {}
    raw = questionnaire.get("raw", "")
    error = questionnaire.get("error")

    items: List[str] = []
    for idx, question in enumerate(questions, start=1):
        key = f"q{idx}"
        answer_text = answers.get(key) or answers.get(question)
        if not answer_text and not answers and raw:
            answer_text = raw
        if answer_text:
            answer_html = render_markdown_block(str(answer_text))
        else:
            answer_html = "<em class='markdown'>No answer provided.</em>"
        items.append(
            "<li>"
    ...
    if not items and error:
        items.append(
            "<li><em class='markdown'>Questionnaire unavailable: "
            f"{html.escape(str(error))}</em></li>"
        )
    ...
    if not items:
        ...
        if raw_text:
            items.append(
                "<li><div class='question-text'>Raw Response</div>"
                f"{render_markdown_block(raw_text)}"
                "</li>"
            )
        else:
            # Last resort: show a friendly note
            items.append(
                "<li><em class='markdown'>No questionnaire response was captured.</em></li>"
            )

    return (
        "<section class='questionnaire-section'>"
        "<h2 class='section-title'>Post-Shift Questionnaire</h2>"
        f"<ol class='questionnaire-list'>{''.join(items)}</ol>"
        "</section>"
    )
