...
def render_function_call_entry(entry: Dict[str, Any]) -> str:
    ...
    if isinstance(parsed_args, dict):
        possible_svg = parsed_args.get("code") or parsed_args.get("svg")
        if isinstance(possible_svg, str) and is_svg_markup(possible_svg):
            svg_html = render_inline_svg(possible_svg)
    elif isinstance(arguments_raw, str) and is_svg_markup(arguments_raw):
        svg_html = render_inline_svg(arguments_raw)
    ...
    header_html = (
        f"<div class='message-header'><span class='role-title'>Draw SVG</span>{info_badge}</div>"
    )
    body_html = svg_html or "<em>No SVG provided.</em>"
    return (
...
def render_message(message: Dict[str, Any], reasoning_html: Optional[str] = None) -> str:
    ...
    header_html = (
        f"<div class='message-header'><span class='role-title'>{html.escape(role.title())}</span>"
        f"{info_badge}</div>"
    )
        ...
        if entry.get("role"):
            ...
            if entry.get("type") == "function_call":
                items.append(
                    ...
                    + render_function_call_entry(entry)
                    + "</div>"
                )
            else:
                items.append(
...
def render_tool_runs(tool_runs: List[Dict[str, Any]]) -> str:
    if not tool_runs:
        return ""
    cards: List[str] = []
    for idx, run in enumerate(tool_runs, start=1):
        title = run.get("tool") or f"Tool Run {idx}"
        summary = html.escape(str(title))
        body = html.escape(json.dumps(run, indent=2, ensure_ascii=False))
        cards.append(
    ...
    return f"<section class='tool-run-section'><h2 class='section-title'>Tool Runs</h2>{''.join(cards)}</section>"
...
def main() -> None:
    ...
    conversation_html = render_conversation(conversation)
    tool_runs_html = render_tool_runs(tool_runs)
